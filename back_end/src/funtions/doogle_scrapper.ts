...
class GoogleLensScraper {
  ...
  private async setupGoogleLensPage(browser: Browser): Promise<Page> {
  ...
  private async scrapeImageDetails(page: Page, imageUrl: string): Promise<ScrapedProduct[]> {
    try {
      ...
      await page.evaluate((url) => {
        const input = document.querySelector('input[placeholder="Paste image link"]') as HTMLInputElement;
      ...
      await page.evaluate(() => {
        const searchButton = document.querySelector('.Qwbd3') as HTMLDivElement;
  ...
  async scrapeProductImages(imageUrls: string[]): Promise<ScrapedProduct[]> {
...
export async function dooglescrappingfunc(image : string) {
